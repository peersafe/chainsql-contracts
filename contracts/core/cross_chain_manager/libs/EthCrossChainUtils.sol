...
import "../interface/IToolsPrecompiled.sol";
...
library ECCUtils {
    ...
    IToolsPrecompiled constant Tools = IToolsPrecompiled(address(0x1002));
    ...
    */
    function _getBookKeeper(uint _keyLen, uint _m, bytes memory _pubKeyList) internal pure returns (bytes20, address[] memory){
         ...
         bytes20  nextBookKeeper = bytes20(Tools.eth_ripemd160(abi.encodePacked(Tools.eth_sha256(buff))));
    ...
    */
    function verifySig(bytes memory _rawHeader, bytes memory _sigList, address[] memory _keepers, uint _m) internal pure returns (bool){
        ...
        for(uint j = 0; j  < sigCount; j++){
            ...
            signers[j] =  ecrecover(Tools.eth_sha256(abi.encodePacked(hash)), v, r, s);
    ...
    */
    function getHeaderHash(bytes memory rawHeader) internal pure returns (bytes32) {
        return Tools.eth_sha256(abi.encodePacked(Tools.eth_sha256(rawHeader)));
    }
}