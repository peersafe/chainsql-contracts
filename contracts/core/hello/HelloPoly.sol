...
contract HelloPoly {
    ...
    function bindProxyHash(uint64 _toChainId, bytes memory _targetProxyHash) public returns (bool) {
        ...
        emit BindProxyEvent(_toChainId, bytes(_targetProxyHash));
        return true;
    }
     ...
     * @return bool
     **/
    function hear(bytes memory _somethingWoW, bytes memory _fromContractAddr, uint64 fromChainId) public returns (bool){
        hearSomeThing = _somethingWoW;
        emit Hear(_somethingWoW, _fromContractAddr);
        return true;
    }

    function something() public view returns(string memory) {
        return string(hearSomeThing);
    }
